...
def get_keep_last(user_id: int) -> int:
    ...
    if not s: return max(1, KEEP_LAST_BOT_MSGS_DEFAULT)
    return max(1, int(s["keep_last_bot_msgs"] if s["keep_last_bot_msgs"] is not None else KEEP_LAST_BOT_MSGS_DEFAULT))
    ...
    async def __call__(self, handler, event, data):
        ...
        try:
            if isinstance(event, Message) and event.chat and event.chat.type == "private":
                if event.from_user and get_delete_user_msgs(event.from_user.id):
...
def extras_more_kb():
    ...
    kb.button(text="📅 Month Close Preview", callback_data="x_month_close")
...
async def dev_tools(c: CallbackQuery):
    try:
        if not only_owner(c): return await c.message.answer(deny_text())
        await wipe_ui(c.message.chat.id, source_message=c.message, keep_last=0)
        await send_text(c.message.chat.id, "🧰 <b>Developer Tools</b>", dev_kb()); await c.answer()
    except Exception as e:
        await c.message.answer(f"❌ dev tools error: {e}")
...
async def extras_more(c: CallbackQuery):
    try:
        if not only_owner(c): return await c.message.answer(deny_text())
        await wipe_ui(c.message.chat.id, c.message, keep_last=0)
        await send_text(c.message.chat.id, "➕ <b>More Extras</b>", extras_more_kb()); await c.answer()
    except Exception as e:
        await c.message.answer(f"❌ extras more error: {e}")
...
async def x_split(c: CallbackQuery, state: FSMContext):
    try:
        if not only_owner(c): return await c.message.answer(deny_text())
        await state.set_state(PinState.waiting_pin)  # reuse a state slot temporarily
        await wipe_ui(c.message.chat.id, c.message, keep_last=0)
        await send_text(c.message.chat.id, "Send: total,people  (e.g., 1200,3)")
        await c.answer()
    except Exception as e:
        await c.message.answer(f"❌ split error: {e}")
...
async def x_split_input(m: Message, state: FSMContext):
    try:
        ...
        if "," not in txt:
            await m.answer("Format: total,people"); return
        ...
        await m.answer(f"❌ split input error: {e}")
    finally:
        await state.clear()
...
async def x_tip(c: CallbackQuery):
    try:
        ...
        await c.message.answer(f"❌ tip error: {e}")
...
async def x_emi(c: CallbackQuery):
    try:
        ...
        await c.message.answer(f"❌ emi error: {e}")

@router.message(F.text.regexp(r"^\d+(?:\.\d+)?,\d+(?:\.\d+)?,\d+$"))
async def x_emi_input(m: Message):
    try:
        if not only_owner(m): return
        p_s, r_s, n_s = [s.strip() for s in m.text.split(",")]
...
@router.callback_query(F.data == "x_month_close")
async def x_month_close(c: CallbackQuery):
    try:
        if not only_owner(c): return await c.message.answer(deny_text())
        total = monthly_total(OWNER_ID) if 'monthly_total' in globals() else 0.0
        await wipe_ui(c.message.chat.id, c.message, keep_last=0)
        await send_text(c.message.chat.id, f"Month-to-date total: {_fmt_currency(float(total))}")
        await c.answer()
    except Exception as e:
        await c.message.answer(f"❌ month close error: {e}")

@router.callback_query(F.data == "x_overdue7")
async def x_overdue7(c: CallbackQuery):
    try:
        if not only_owner(c): return await c.message.answer(deny_text())
        rows = list_overdues(OWNER_ID, days=7) if 'list_overdues' in globals() else []
        msg = "\n".join(rows) if isinstance(rows, list) else str(rows)
        await wipe_ui(c.message.chat.id, c.message, keep_last=0)
        await send_text(c.message.chat.id, msg or "No overdues >7d"); await c.answer()
    except Exception as e:
        await c.message.answer(f"❌ overdue7 error: {e}")

@router.callback_query(F.data == "x_top")
async def x_top(c: CallbackQuery):
    try:
        if not only_owner(c): return await c.message.answer(deny_text())
        top = top_debtors(OWNER_ID, 5) if 'top_debtors' in globals() else []
        text = "\n".join(top) if isinstance(top, list) else str(top)
        await wipe_ui(c.message.chat.id, c.message, keep_last=0)
        await send_text(c.message.chat.id, text or "(empty)"); await c.answer()
    except Exception as e:
        await c.message.answer(f"❌ top debtors error: {e}")

@router.callback_query(F.data == "x_small")
async def x_small(c: CallbackQuery):
    try:
        if not only_owner(c): return await c.message.answer(deny_text())
        small = smallest_balances(OWNER_ID, 5) if 'smallest_balances' in globals() else []
        text = "\n".join(small) if isinstance(small, list) else str(small)
        await wipe_ui(c.message.chat.id, c.message, keep_last=0)
        await send_text(c.message.chat.id, text or "(empty)"); await c.answer()
    except Exception as e:
        await c.message.answer(f"❌ smallest balances error: {e}")

@router.callback_query(F.data == "x_export_json")
async def x_export_json(c: CallbackQuery):
    try:
        if not only_owner(c): return await c.message.answer(deny_text())
        path = export_json_snapshot(OWNER_ID) if 'export_json_snapshot' in globals() else None
        if path:
            await send_document(c.message.chat.id, Path(path), "JSON export")
        else:
            await send_text(c.message.chat.id, "No exporter defined")
        await c.answer()
    except Exception as e:
        await c.message.answer(f"❌ export json error: {e}")

@router.callback_query(F.data == "x_datacheck")
async def x_datacheck(c: CallbackQuery):
    try:
        if not only_owner(c): return await c.message.answer(deny_text())
        msg = run_data_check(OWNER_ID) if 'run_data_check' in globals() else "OK"
        await wipe_ui(c.message.chat.id, c.message, keep_last=0)
        await send_text(c.message.chat.id, f"Data check: {msg}"); await c.answer()
    except Exception as e:
        await c.message.answer(f"❌ datacheck error: {e}")
